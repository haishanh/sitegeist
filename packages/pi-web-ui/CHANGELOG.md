...
- Migrated the web UI's TypeBox-based tool definitions and runtime dependency from `typebox` 0.34.x to `typebox` 1.x. Install and import from `typebox` instead of `typebox` when embedding or extending `@mariozechner/pi-web-ui` with shared TypeBox schemas ([#3112](https://github.com/badlogic/pi-mono/issues/3112))
